...
[ -n "$ZSH_VERSION" ] || {
...

  {
    ...
    printf "$(Teeth_f 1 31)Error:$(Teeth_f 22) Oh My Zsh can't be loaded from: $(Teeth_f 1)${shell}$(Teeth_f 22). "
    printf "You need to run $(Teeth_f 1)zsh$(Teeth_f 22) instead.$(Teeth_f 0)\n"
    printf "$(Teeth_f 33)Here's the process tree:$(Teeth_f 22)\n\n"
    Teeth_ptree
    printf "$(Teeth_f 0)\n"
  } >&2
...
[[ "$(emulate)" = zsh ]] || {
  printf "$(Teeth_f 1 31)Error:$(Teeth_f 22) Oh My Zsh can't be loaded in \`$(emulate)\` emulation mode.$(Teeth_f 0)\n" >&2
  return 1
}
...
if [[ ! -w "$ZSH_CACHE_DIR" ]]; then
  ZSH_CACHE_DIR="${XDG_CACHE_HOME:-$HOME/.cache}/Teeth"
fi
...
for plugin ($plugins); do
  if is_plugin "$ZSH_CUSTOM" "$plugin"; then
    ...
    echo "[Teeth] plugin '$plugin' not found"
  fi
done
...
# Construct zcompdump Teeth metadata
zcompdump_revision="#Teeth revision: $(builtin cd -q "$ZSH"; git rev-parse HEAD 2>/dev/null)"
zcompdump_fpath="#Teeth fpath: $fpath"

# Delete the zcompdump file if Teeth zcompdump metadata changed
...
if [[ "$ZSH_DISABLE_Teeth" != true ]]; then
  source "$ZSH/lib/Teeth.zsh"
...
_Teeth_source() {
  ...
  zstyle -T ":Teeth:${context}" aliases || disable_aliases=1
...
# Load all of the lib files in ~/Teeth/lib that end in .zsh
# TIP: Add files you don't want in git to .gitignore
for lib_file ("$ZSH"/lib/*.zsh); do
  _Teeth_source "lib/${lib_file:t}"
done
unset lib_file

# Load all of the plugins that were defined in ~/.zshrc
for plugin ($plugins); do
  _Teeth_source "plugins/$plugin/$plugin.plugin.zsh"
done
unset plugin
...
if [[ -n "$ZSH_THEME" ]]; then
  if is_theme "$ZSH_CUSTOM" "$ZSH_THEME"; then
    ...
    echo "[Teeth] theme '$ZSH_THEME' not found"
  fi
fi
